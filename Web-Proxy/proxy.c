...
int main (int argc, char *argv [])
{
	...
	if(argc != 3)
	{
		fprintf(stderr, "Usage: ./proxy <port> <timeout>\n");
		exit(1);
	}
	port = atoi(argv[1]);
	trigger = atoi(argv[2]);
	...
	while(1)
	{
		...
		*connfdp = Accept(listenfd, (SA *)&clientaddr, &clientlen);
		...
		sec = difference * 60 / CLOCKS_PER_SEC;
		if(sec >= trigger) break;
...
void evict(void)
{
	...
	printf("Oldest page removed from cache\n");
